...
Let $\pi^i_\sigma$ be the proportion of individuals of type $\sigma \in \{s, r, g\}$ in
group $i$. Let $\pi_\sigma$ be the overall proportion of type $\sigma$ in the population.

Write $h^t_{ij}$ for the probability that an individual from group $i$ helps
...
previously helped by an actor from group $j$. Setting $h^0_{ij} = 1$ for all $i,
...
Denote by $X \subset \mathcal{G}$ the set of groups $i$ which contain *only* 
...
Now apply the above argument to $h_{ki}$ for any $k \notin X$. This shows $\pi^k_s = 0$.
But for $k \in X$, $\pi^k_s = 0$ by definition. Thus $\pi_s = 0$. Contradiction. This
shows $h_{ik} = 0$ for all $i \notin X$ and all $k$. Finally, if $i \in X$ and $k \notin X$,
$h_{ik} = h_{ki}$ as noted above, and this is 0. QED.
