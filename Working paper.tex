...
\label{prop:decrease_in_p}
    ...
    % The LHS of equation \eqref{gr-wins-2} is \sout{de}\mm{in}creasing in $p=\mm{\frac{\Sigma_gl_g}{N}}$ \sout{and is equal to the threshold $k$ when $p = 0$}.\footnote{\sout{When the share of GR in the population is very small, the probability that $p_g = k$ conditional on~$p_g\geq k$ goes to one.}}
    Randomly choosing a selfish type to be replaced by a GR type without changing the group assignment (weakly) increases the expected value of $\frac{x}{q}$.}
\end{prop}
...
\begin{proof}
    ...
    \mm{
    % A marginal increase in $p$ is modeled by a random selection of a selfish type who is replaced by a GR-type. That is, w
