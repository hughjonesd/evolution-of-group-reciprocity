...
that humans hold attitudes towards entire groups \parencite{brewer1985psychology,neuberg2008intergroup}.
...
\begin{itemize}
    \item Group reciprocators in supraliminal groups get a payoff of~$(\bpi qb - qc)\mm{N}$ \mm{$= \Sigma_{l_g =k}^n[l_gb-nc]$}.
    \item Selfish types in supraliminal groups get~$\bpi qb\mm{N}$ \mm{$= \Sigma_{l_g =k}^nl_gb$}.
...
\mm{=\frac{\Sigma_{l_g =k}^nl_g \Sigma_{l_g =k}^n[l_gb-nc]}{\Sigma_{l_g =0}^nl_g}}
\end{equation*}
...
\mm{=\frac{\Sigma_{l_g =k}^n(n-l_g) \Sigma_{l_g =k}^nl_gb}{\Sigma_{l_g =0}^n(n-l_g)}}
\end{equation*}
...
\mm{\Longleftrightarrow \frac{N\Sigma_{l_g =k}^nl_g-M\Sigma_{l_g =0}^nl_g}{N-M\Sigma_{l_g =0}^nl_g} \ge M\frac{b}{c}}.
\end{equation}

where \mm{$M \equiv qN=\Sigma_{l_g =k}^nn$ and 

\[
\bpi = \frac{\Sigma_{l_g =k}^nl_g}{M}
\]}
